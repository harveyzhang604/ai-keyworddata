...
服务器端数据接入完整指南 - 关键词挖掘系�?API

---

## 概述

本系统提�?3 个核�?API 端点，用于服务器端上传关键词挖掘数据�?

| 方法 | 端点 | 说明 |
|------|------|------|
| POST | `/api/ingest/runs` | 创建挖掘任务 |
| POST | `/api/ingest/keywords/batch` | 批量上传关键�?|
...
所�?API 请求必须在请求头中包�?API Key，支持两种格式：
...
|------|------|------|------|
| `seed` | string | �?| 种子关键�?|
| `rounds` | number | �?| 挖掘轮数，默�?1 |
| `meta` | object | �?| 元数�?(JSON)，可存储任意扩展信息 |
  ...
  "keywords": [
    {
      ...
      "pain_point_flag": false,
        "pain_point": "User cannot export PDF to Excel cleanly",
      "raw_data": {
        "cpc": 2.5,
        "competition": 0.65,
        "trend": "stable",
...
| 字段 | 类型 | 必填 | 说明 | 存储位置 |
|------|------|------|------|----------|
| `keyword` | string | �?| 关键词文�?| keywords �?|
| `score` | number | �?| 综合得分 (0-100) | observations �?|
| `search_volume` | number | �?| 月搜索量 | observations �?|
| `difficulty` | string | �?| 难度: `low`, `medium`, `high` | observations �?|
| `intent` | string | �?| 意图: `informational`, `navigational`, `commercial`, `transactional` | observations �?|
| `source` | string | �?| 数据来源: `ahrefs`, `semrush`, `google` �?| observations �?|
| `word_count` | number | �?| 词数 | observations �?|
| `pain_point_flag` | boolean | �?| 是否为痛点词，默�?false | observations �?|
| `pain_point` | TEXT | 痛点描述 |
| `language` | string | �?| 语言代码 (�?`en`) | keywords �?|
| `country` | string | �?| 国家代码 (�?`US`) | keywords �?|
| `category` | string | �?| 分类标签 | keywords �?|
| `raw_data` | object | �?| **原始数据 (JSON)，用于存储任意扩展字�?* | observations �?|
...
1. **关键词去�?*: 基于标准化关键词 (`keyword_norm` = 小写 + 去多余空�?
2. **观察记录去重**: 同一 `run_id` 下相同关键词不会重复插入
3. **跨任务不去重**: 不同 `run_id` 可以有相同关键词的不同观察数�?
...
for i in range(0, len(keywords), batch_size):
    ...
    if result["success"]:
        print(f"批次 {i//batch_size + 1}: 插入 {result['inserted']} 个，重复 {result['duplicates']} �?)
    else:
        print(f"错误: {result['error']}")
...
| `json_data` | object | �?| 结构化数�?(JSON) |
| `status` | string | �?| 任务最终状�? `success` �?`failed`，默�?`success` |
...
> **注意**: 上传报告后，系统会自动更新任务状�?(`status`) 和结束时�?(`ended_at`)�?
...
# 生成 Markdown 报告
markdown_report = f"""# AI Tools 关键词挖掘报�?

**生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M')}

## 📊 统计概览
- 总关键词�? {total_keywords}
- 高价值机�? {high_opportunity}
- 平均得分: {avg_score}
...
| 状态码 | 错误类型 | 说明 | 解决方法 |
|--------|----------|------|----------|
| 401 | Unauthorized | API Key 无效或缺�?| 检�?API Key 是否正确，是否在请求头中 |
| 400 | Bad Request | 请求参数错误或缺失必填字�?| 检�?`seed`、`run_id`、`keywords` 等必填字�?|
| 404 | Not Found | run_id 不存在或不属于当前服务器 | 确认 run_id 是否正确，是否由当前 API Key 创建 |
| 500 | Internal Server Error | 服务器内部错�?| 检查请求体格式，联系管理员 |
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200 or response.status_code == 201:
                ...
                raise Exception("API Key 无效，请检查配�?)
...
for i in range(0, len(keywords_data), batch_size):
    ...
    if result["success"]:
        ...
        print(f"  �?批次 {i//batch_size + 1}: 插入 {result['inserted']} �?)
    else:
        print(f"  �?批次 {i//batch_size + 1} 失败: {result['error']}")
    
    time.sleep(0.5)  # 避免请求过快

print(f"�?上传完成: 插入 {total_inserted} 个，重复 {total_duplicates} �?)
...
## 统计概览
- 总关键词�? {len(keywords_data)}
- 高分关键�?(�?0): {len(high_score_keywords)}
- 平均得分: {avg_score:.1f}
...
1. **批量上传**: 每批 100-500 个关键词，避免单次请求过�?
2. **请求间隔**: 批次之间添加 500ms 延迟，避免触发速率限制
3. **错误重试**: 使用 try-except 捕获网络错误，实现自动重�?
4. **保存 run_id**: 必须保存，后续上传关键词和报告都需�?

### 数据质量

5. **关键词去空格**: 系统会自动标准化，但建议上传前清�?
6. **扩展字段�?raw_data**: `cpc`、`competition`、`trend` 等字段放�?`raw_data`
7. **提供 score**: 虽然可选，但强烈建议计算并提供综合得分

### 报告生成

8. **双格式报�?*: 同时提供 `markdown` �?`json_data`，便于展示和分析
9. **明确 status**: 任务成功设为 `success`，失败设�?`failed`
...
| `miner_id` | INTEGER | 自动关联服务�?ID（通过 API Key�?|
| `seed` | VARCHAR(255) | 种子关键�?|
| `rounds` | INTEGER | 挖掘轮次 |
| `status` | VARCHAR(50) | 状�? `running` �?`success`/`failed` |
| `started_at` | TIMESTAMP | 创建时自动设�?|
| `ended_at` | TIMESTAMP | 上传报告时自动设�?|
| `meta_json` | TEXT | 存储 `meta` 对象�?JSON 字符�?|
...
| `report_json` | TEXT | 存储 `json_data` �?JSON 字符�?|
...
```mermaid
sequenceDiagram
    participant Server as 挖掘服务�?
    participant API as API 服务
    participant DB as 数据�?

    Note over Server: 开始挖掘流�?
